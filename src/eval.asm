...
ASC0:	
	LD A,(INKEYDH)
	BIT 7,A         ;negative timeout means we are checking for a key
	JR Z, TIME_OUT
	LD A,(INKEYDL); KEY CODE
	LD L,A
	LD	A,(ACCS) ; SI SE PULSA HL ES LA TECLA PULSADA
	NEG A
	CP L
	JP Z,TRUE	
	JP FALSE
TIME_OUT: 
	...
	XOR	A
	CP	E
	JP	Z,TRUE		;NULL STRING CUANDO NO RECIBE TECLA?¿? NO DEBERIA DEVOLVER 0?
ASC1:	LD	HL,(ACCS) ; SI SE PULSA HL ES LA TECLA PULSADA
	JR	COUNT0
LEN:	CALL	ITEMS
	EX	DE,HL
	JR	COUNT0
